...
export default function Home({ searchParams }) {
  const activeLane = searchParams?.lane || 'INTERNATIONAL';
  ...
  const handleReceiptButtonClick = () => {
    ...
    setLoadingAd(true);
    // Mandatory 5-Second Video Advertisement Simulation Interface
    setTimeout(() => {
      setLoadingAd(false);
      setAdWatched(true);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '20px', color: '#0B0B0F', backgroundColor: '#F4F4F6' }}>
      ...
      {/* Feature Section Box - Clean White Card Layout */}
      <div style={{ backgroundColor: '#FFFFFF', borderRadius: '16px', padding: '32px', textAlign: 'center', border: '1px solid #D1D1D6', width: '100%', maxWidth: '360px', marginBottom: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
        ...
        {/* Interaction Button Element - Highlights with Royal Green on Watch Ad state */}
        ...
        <input id="receipt-file-input" type="file" accept="image/*" disabled={!adWatched} style={{ display: 'none' }} onChange={(e) => alert(`File staging complete: ${e.target.files?.name}`)} />
      </div>

      {/* Pricing Configuration Card Component */}
      <div style={{ backgroundColor: '#FFFFFF', borderRadius: '16px', padding: '40px', width: '100%', maxWidth: '360px', border: '1px solid #D1D1D6', textAlign: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
        ...
        {/* Primary Call to Action Link Element - Solid Royal Green Contrast Highlight */}
